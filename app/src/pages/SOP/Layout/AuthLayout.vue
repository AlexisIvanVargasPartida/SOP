    ...
    <md-toolbar md-elevation="0" class="md-transparent md-toolbar-absolute">
      <div class="md-toolbar-row md-offset">
        <div class="md-toolbar-section-start">
          <h3 class="md-title">{{ $route.meta.title || "SOP" }}</h3>
        </div>
        <div class="md-toolbar-section-end">
          ...
          >
            <md-list>
              <md-list-item
                to="/registro"
                v-if="$route.path != '/registro' && 1 == 2"
              >
                <md-icon>person_add</md-icon>
                Registro
              </md-list-item>
              <md-list-item to="/login" v-if="$route.path != '/login'">
...
export default {
  ...
  computed: {
    setBgImage() {
      let images = {
        Login: "./img/bg-pricing.jpg",
        Registro: "./img/banner.jpg"
      };
      let back = {
        background: `linear-gradient(135deg, rgb(98 184 102) 0%, rgb(5 90 7) 100%)`
      };
      return back;
      //if (this.$route.name == "Login") return back;
      return {
